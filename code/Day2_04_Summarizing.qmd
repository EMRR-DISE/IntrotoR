...
## Pivoting (not finished)

In all of our examples, we've had each analyte as a column header. some cases, however, it would be more advantageous for all analyte names to be in one column with their values in another. How would we achieve this?

What we want to do is *pivot* the rows and columns. `tidyverse` has two handy functions for this: `pivot_longer` and `pivot_wider`. 
...
We can see that our data shape has changed:
...
We see that we went from 20 columns to 4. However, we also went from 62 rows to 1116. This is because there are 62 unique dates and each of the 18 analytes has a row for each one. Your data and goals will dictate which structure is more advantageous; sometimes you even need both!

For now, though, say we decided we want to pivot back to our old way. Here, we are transforming two columns into >2 columns, making the dataset wider. Hence, `pivot_wider`. Here, instead of pivoting to columns, we pivot from them, and need:
- which column to take the names from
