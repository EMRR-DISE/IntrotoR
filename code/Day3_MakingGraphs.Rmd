...
knitr::opts_knit$set(root.dir = 'C:/Repositories/IntrotoR')
getwd()
```
...
```{r read in data}
setwd('C:/Repositories/IntrotoR')
wqdata <- read_csv("./data/WQ_P8D7.csv")
...
(plot_ts_points_lines2 <- ts_points +
  geom_line())

```
Now it is a little easier to see the patterns through time. We can see that Secchi depth is low in the winter and high in the summer.  
...
#time series plot with connecting lines for both stations customized
(plot_ts_points_lines_two_custom <- ggplot(data = wqdata, aes(x = Date, y = Secchi, group = Station)) +
  geom_point(aes(color = Station))+
  geom_line(aes(color = Station)))+
