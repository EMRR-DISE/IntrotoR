...
So without further adue, let's load the packages we need for today's tutorial. In addition to 'ggplot2', we will also load the tidyverse to do some basic tasks like reading in data sets and filtering data.
...
The 'ggplot2' package utilizes the grammer of graphics and creates plots in "layers". We will learn about some of the core "geometries" which are basically just types of plots. 
...
Based on the code above for the water temperature histogram, make a histogram of SpCndSurface and try different numbers of bins.
...
```
This plot clearly shows what we noted in the individual plots of Secchi depth, namely that Secchi depth is generally much higher at P8 than D7.
...
Look at the correlation between Chla and Pheophytin. 

```{r}
(plot_scatter_phyto <- ggplot(data = wqdata, aes(x = Chla, y = Pheophytin)) +
  geom_point()+
  geom_smooth(method = lm)
)
...
```
Chla and Pheophytin are not correlated.
...
Microcystis needs high temperatures to become abundant. It also needs a lot of light. Plot Microcystis against Secchi depth to see whether turbidity related to Microcystis abundance. Include the points for the raw data.
...
Quick references for the customizations we will discuss
Colors [https://r-charts.com/colors/]
Point shapes ![Point shapes.](/images/PointShapes.png)
Line types ![Line types.](/images/LineTypes.png)
